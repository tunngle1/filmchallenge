(function () {
    'use strict';
    Lampa.Noty.show('🚀 V13 (Scroll Fix 2) Загружен');
    // ========================================
    ...
    function markAsWatched(movie, watchedAt, callback) {
        ...
        var movieData = {
            ...
            rating: movie.rating || null,
            comment: movie.comment || null,
            genres: movie.genres || [] // Сохраняем жанры
        };
    ...
    function getRandomGlobal(callback) {
        ...
        // V10: Гибрид. Используем прокси Лампы, но явно добавляем ключ.
        // Lampa.TMDB.api вернет прокси-адрес, но сохранит наши параметры.
        var params = 'movie/popular?api_key=' + apiKey + '&language=ru&page=' + page;
        var url = Lampa.TMDB.api(params);

        console.log('[Challenge365] V10 URL:', url);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.results && data.results.length) {
                    var r = data.results[Math.floor(Math.random() * data.results.length)];
                    r.source = 'tmdb'; if (!r.poster_path) r.img = '';
                    callback(r);
                } else { Lampa.Noty.show('Пусто'); }
            },
            error: function (xhr, status, error) {
                Lampa.Loading.stop();
                Lampa.Noty.show('V10 Error: ' + xhr.status + ' | ' + error);
            }
        });
    }
    ...
    function getStatistics(callback) {
        ...
        // Считаем среднюю оценку и жанры
        var totalRating = 0;
        var ratingCount = 0;
        var genreCounts = {};

        Object.keys(PluginData.movies).forEach(function (id) {
            var m = PluginData.movies[id];
            if (m.rating) {
                totalRating += parseInt(m.rating);
                ratingCount++;
            }
            if (m.genres && Array.isArray(m.genres)) {
                m.genres.forEach(function (g) {
                    var name = g.name || g; // Lampa может давать объект или строку
                    if (genreCounts[name]) genreCounts[name]++;
    ...
    function showStatistics() {
        getStatistics(function (stats) {
            ...
            var rows = [
                ...
                { label: 'Средняя оценка', value: '⭐ ' + stats.avg_rating },
                { label: 'Любимый жанр', value: stats.top_genre },
    ...
    function registerHistoryComponent() {
        Lampa.Component.add('challenge365_history', function () {
            ...
            this.create = function () {
                ...
                // V13: Улучшенный скролл (поддержка mousewheel и wheel)
                scroll.render().on('mousewheel wheel', function (e) {
                    var delta = e.originalEvent.wheelDelta || -e.originalEvent.deltaY || -e.originalEvent.detail;

                    // Если delta отрицательная -> крутим вниз -> Navigator.move('down')
                    // Если delta положительная -> крутим вверх -> Navigator.move('up')
                    if (delta < 0) Navigator.move('down');
