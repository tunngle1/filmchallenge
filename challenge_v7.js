(function () {
    'use strict';
    Lampa.Noty.show('🚀 V7 (Global Fix) Загружен');
    // ========================================
    ...
    function getRandomGlobal(callback) {
        ...
        // Берем случайную страницу популярных фильмов (1-50)
        var page = Math.floor(Math.random() * 50) + 1;
        var url = Lampa.TMDB.api('movie/popular?language=ru&page=' + page);

        // Используем стандартный ajax запрос
        $.ajax({
            url: url,
            dataType: 'json',
            ...
            error: function () {
                Lampa.Loading.stop();
                Lampa.Noty.show('Ошибка поиска. Попробуйте еще раз.');
            }
        });
    }
