(function () {
    'use strict';
    Lampa.Noty.show('🚀 V8 (Debug) Загружен');
    // ========================================
    ...
    function getRandomGlobal(callback) {
        ...
        // Самый простой запрос (Тренды недели), чтобы исключить ошибку в параметрах
        var url = Lampa.TMDB.api('trending/movie/week?language=ru');

        console.log('[Challenge365] URL:', url); // Для отладки в консоли F12
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                Lampa.Loading.stop();
                // Показываем ТОЧНЫЙ код ошибки на экране
                Lampa.Noty.show('Error: ' + xhr.status + ' | ' + error);

                // Пробуем запасной вариант - Fetch (если ajax блокируется)
                fetch(url).then(r => r.json()).then(data => {
                    if (data.results) {
                        Lampa.Noty.show('Сработало через Fetch!');
                        var r = data.results[0]; // Берем первый для теста
                        r.source = 'tmdb'; if (!r.poster_path) r.img = '';
                        callback(r);
                    }
                }).catch(e => Lampa.Noty.show('Fetch тоже не смог: ' + e.message));
            }
        });
    }
