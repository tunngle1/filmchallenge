(function () {
    'use strict';
    Lampa.Noty.show('🚀 V9 (API Key) Загружен');
    // ========================================
    ...
    function getRandomGlobal(callback) {
        ...
        // 1. Генерируем страницу
        var page = Math.floor(Math.random() * 50) + 1;

        // 2. Публичный ключ TMDB (Lampa использует его же во многих плагинах)
        var apiKey = '4ef0d7355d9ffb5151e987764708ce96';

        // 3. Формируем ПОЛНЫЙ URL с ключом
        var url = 'https://api.themoviedb.org/3/movie/popular?api_key=' + apiKey + '&language=ru&page=' + page;

        console.log('[Challenge365] V9 URL:', url);
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                Lampa.Loading.stop();
                Lampa.Noty.show('V9 Error: ' + xhr.status + ' | ' + error);
            }
        });
    }
